...
class YOLOPAFPN(nn.Module):
    ...
    def forward(self, input, ttt_prob=0.0, ttt_noise_scale=1.0):
        """
        Args:
            inputs: input images.
            ttt_prob: Probability of running TTT (passed from YOLOX).
            ttt_noise_scale: Dynamic noise scaler (passed from YOLOX).
        """

        # Pass both arguments down to CSPDarknet
        out_features = self.backbone(input, ttt_prob=ttt_prob, ttt_noise_scale=ttt_noise_scale)

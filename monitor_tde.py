...
class TDEForensicParser:
    def __init__(self, log_path):
        ...
        # --- CRITICAL FIX: Regex updated to capture 'proto_loss' ---
        # Note the addition of 'proto_loss: ([\d.]+)'
        self.train_re = re.compile(
            r"epoch: \[(\d+)/\d+\]\[(\d+)/\d+\], mem: (\d+)Mb,.*?total_loss: ([\d.]+), iou_loss: ([\d.]+), l1_loss: ([\d.]+), conf_loss: ([\d.]+), cls_loss: ([\d.]+), mem_loss: ([\d.]+), proto_loss: ([\d.]+), ttt_prob: ([\d.]+), lr: ([\d.e-]+)"
        )
        ...
        while i < len(lines):
            ...
            if train_match:
                self.train_records.append({
                    ...
                    "proto_loss": float(train_match.group(10)), # <-- Extracted
...
def generate_report(parser, base_name):
    ...
    # [2,0] *** NEW: PROTO_SIM (Identity Separation) ***
    if 'proto_loss' in df_t.columns:
        sns.lineplot(data=df_t, x='epoch', y='proto_loss', ax=axes[2,0], color='dodgerblue', linewidth=3)
        axes[2,0].set_title("Memory Bank Orthogonality (proto_loss $\\to$ 0.0 is better)", fontsize=14)
        axes[2,0].set_ylim(-0.1, 1.1) # Cosine similarity bound
        axes[2,0].axhline(0.0, ls='--', color='gray', alpha=0.7)
